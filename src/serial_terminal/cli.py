...
def parse_args(arguments: list[str] | None = None) -> tuple[list[str], PortSettings, Path]:
    ...
    if args.baudrate <= 0 or args.timeout <= 0:
        build_parser().error("--baudrate and --timeout must be greater than zero")
    return args.ports, PortSettings(args.baudrate, args.bytesize, args.parity, args.stopbits, args.timeout), args.log_dir
